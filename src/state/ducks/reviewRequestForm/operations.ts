...
async function getAllUserFetch() {
    ...
    let users: User[] = [];
    result.userMany.map((user: { _id: string; firstName: string; lastName: string; }) => {
        let userModel: User = {
            id: user._id,
...
async function getAllCheckListsFetch() {
    ...
    result.checkListMany.map((checkList: { _id: string; title: string}) => {
        let checkListModel: CheckList = {
            id: checkList._id,
            title: checkList.title
        }

        checkLists.push(checkListModel);
    });
...
async function putReviewInfoFetch(reviewInfo: ReviewInfo) {
    let members: {_id: string, role: number}[] = [];
    reviewInfo.reviewerList!.map(memberInfo =>
        { members.push({ _id: memberInfo.member.id, role: memberInfo.memberType }) });
    reviewInfo.revieweeList!.map(memberInfo =>
        { members.push({ _id: memberInfo.member.id, role: memberInfo.memberType }) });

    let valiable: CreateReviewInfoQueryType = {
        title: reviewInfo.title!,
        path: reviewInfo.documentPath!,
        tags: reviewInfo.tags,
        checkLists: reviewInfo.checkListIds,
        members: members,
        status: 0,
        scope: reviewInfo.scope!
    }

    let result = await graphqlClient.request(createReviewInfoQuery, valiable);
